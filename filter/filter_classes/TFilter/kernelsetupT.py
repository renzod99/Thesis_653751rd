...
def extract_descriptor_metadata(descriptor) -> Dict[Tuple[str, int], List[str]]:
    ...
    metadata[("measurement variance", 1)] = ["R", "nu"]
    return metadata
...
def reconstruct_kernels(param_keys, params):
    """
    converts the hyperparameters found after optimization back into a dictionary, using the found hyperparameters in optimization
    """
...
def construct_kernel_ss_mats(descriptor, hyperparams):
    ...
    # ss_mats["gamma"] = hyperparams['measurement variance', 1]["gamma"]
    ss_mats["nu"] = hyperparams['measurement variance', 1]["nu"]
    return ss_mats
